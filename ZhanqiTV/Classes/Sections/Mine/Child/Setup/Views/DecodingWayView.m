...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (i == 0) {
                button.frame = CGRectMake(0, 0, self.buttonView.width, buttonHeight);
                lineView.frame = CGRectMake(0, button.bottom, button.width, 0.5);
            }
            else if (i == 1)
            {
                button.frame = CGRectMake(0, buttonHeight+0.5, self.buttonView.width, buttonHeight);
                lineView.frame = CGRectMake(0,button.bottom, button.width,10);
            }
            else
            {
                button.frame = CGRectMake(0, buttonHeight*2+10+0.5, self.buttonView.width, buttonHeight);
                lineView.frame = CGRectMake(0, button.bottom, button.width, 0);
            }
            button.tag = i;
...
{
    if (!_buttonView) {
        _buttonView = [[UIView alloc]initWithFrame:CGRectMake(0, self.height, self.width, 44*3+0.5+10)];
...
- (void)appearView
{
    ...
    [UIView animateWithDuration:1.0 animations:^{
        self.buttonView.frame = CGRectMake(0, self.height-self.buttonView.height, self.buttonView.width, self.buttonView.height);
...
- (void)hideView
{
    [UIView beginAnimations:nil context:nil];
    [UIView animateWithDuration:1.0 animations:^{
        self.buttonView.frame = CGRectMake(0, self.height, self.buttonView.width, self.buttonView.height);
